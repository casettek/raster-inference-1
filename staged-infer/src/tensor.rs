...
impl Slab {
    ...
    pub(crate) fn from_rows(rows: Vec<Vec<i32>>) -> Result<Self> {
    ...
    pub(crate) fn row(&self, row_idx: usize) -> &[i32] {
        &self.data[row_idx * self.cols..(row_idx + 1) * self.cols]
    }

    pub(crate) fn row_mut(&mut self, row_idx: usize) -> &mut [i32] {
        &mut self.data[row_idx * self.cols..(row_idx + 1) * self.cols]
    }

    pub(crate) fn iter_rows(&self) -> std::slice::Chunks<'_, i32> {
        self.data.chunks(self.cols.max(1))
    }

    pub(crate) fn as_flat_mut(&mut self) -> &mut [i32] {
        &mut self.data
    }
}
...
impl Matrix {
    pub(crate) fn from_region<const P: u64>(
        name: &str,
        region: &Bytes<P>,
        rows: usize,
        cols: usize,
...
pub(crate) fn dot_bits(a: &[i32], b: &[i32]) -> i32 {
    let acc = a
        .iter()
        .zip(b)
...
pub(crate) fn mac_weighted_value(acc: &mut i64, weight: i32, value: i32) {
    *acc = mac_bits(*acc, weight, value);
}

pub(crate) fn requantize_acc(acc: i64) -> i32 {
    requantize(Acc::from_bits(acc)).to_bits()
}
