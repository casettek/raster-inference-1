...
`claim build` runs the staged-infer checkpointed path without selecting a
...
`ReplayPackage`, and `ChallengeBundle` when a mismatch is found. Today `infer`
emits an `InferenceResult` through the artifactless staged fallback; the
separate `direct-infer` crate is the scaffold for the future true unstaged
runtime. `FaultProof` is still a planned workflow output.
...
inference-artifacts/        # shared inference/claim/challenge artifact contracts
...
The workflow CLI is the developer-facing entry point for this repo:
...
# build a proposer claim with all staged-infer checkpoints
cargo run --release --manifest-path raster-inference-cli/Cargo.toml -- claim build
...
# run the current artifactless staged fallback without checkpoint artifacts
cargo run --release --manifest-path raster-inference-cli/Cargo.toml -- infer
```

`claim build` currently emits three stable files in the staged-infer chain run
directory:
...
`infer` prints the generated token IDs, token ID hash, stop reason, generated
text, and a timing summary. For now it uses the staged artifactless fallback:
the imported workspace's `Raster.toml` is still the input contract, but no
checkpoint artifacts are written under `target/staged-infer/`. The `direct-infer`
crate exists so a later pass can build a truly unstaged runtime without Raster
storage/materialization boundaries.

The fault-proof command is intentionally reserved until its artifact producer
exists. For now, lower-level Raster and staged-infer commands remain
...
# staged-infer backend entry point used by claim build
cargo run --manifest-path staged-infer/Cargo.toml -- chain run
```
